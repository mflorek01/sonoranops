...
describe("demo operations adapter", () => {
  it("keeps lifecycle changes behind the OperationsApi contract", async () => {
    ...
    const after = await demoApi.transitionIncident("inc-2048", { state: "mitigated", reason: "Controlled test transition" });
  ...
  it("requires an auditable reason for lifecycle changes", async () => {
    await expect(demoApi.transitionIncident("inc-2048", { state: "resolved", reason: "" })).rejects.toThrow("reason is required");
  });

  it("returns labeled deterministic evidence through the assistant boundary", async () => {
    const result = await demoApi.runAssistantTool("get_incident_evidence", { incident_id: "inc-2048" });
